...
import { CJitSimulator } from './CJitSimulator';
...
export class CExecutor implements BaseExecutor {
  async execute(options: ExecutionOptions): Promise<ExecutionResult> {
    ...
    // 1. Compilation Phase (Try gcc, clang, cl)
    let compiled = false;
    let lastCompileError = '';

    const compilerCommands = [
      `gcc -std=c17 "${sourcePath}" -o "${exePath}"`,
      `clang -std=c17 "${sourcePath}" -o "${exePath}"`,
      `cl.exe /std:c17 "${sourcePath}" /Fe"${exePath}"`
    ];
    ...
    if (!compiled) {
      ...
      if (isMissingCompiler) {
        ...
        return await CJitSimulator.execute(options);
      }
